...
def create_memory_optimized_device_map(num_layers: int,
    ...
    embedding_device_idx = 2 if len(available_devices) > 2 else 0  # Put on third device for better balance
    output_device_idx = 1 if len(available_devices) > 1 else 0  # Put on second device
    ...
    device_loads[embedding_device_idx] += 2.5
    device_loads[output_device_idx] += 2.0
...
def get_recommended_batch_size(device_map: Dict[str, torch.device], 
    ...
    # Rule of thumb: model + optimizer states ≈ 3x model size
